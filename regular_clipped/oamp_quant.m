...
M = L * log2(B) / R;
z_full = dct(x);
quant_max = prctile(z_full, 99.99);
[z_full_quant, quant_levels,quant_bounds] = uniform_quantizer(z_full, numLevels, quant_max);
% rng("default");
order = randperm(N, M);
z = z_full_quant(order);
% add noise
[y, ~, sigma] = linear_model_clip(z,SNR);   
